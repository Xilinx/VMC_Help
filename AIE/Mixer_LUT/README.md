...
DESCRIPTION:
Model: Mixer_LUT_Ex1
Generated: 13-Jan-2026 15:37:30
...
Model Annotations:
  ...
  Phase Increment: (Fo*(2^N)) / Fs:  2^32*(100e3/4e6)
  Input window size: 1024
  Input sampling frequency: 4MHz
  SFDR: 80
  Mixer mode: Single Input Mode
...
% Quick Start: Mixer_LUT_Ex1
% Essential setup focusing on critical parameters

modelName = 'Mixer_LUT_Ex1';
...
DESCRIPTION:
Model: Mixer_LUT_Ex2
Generated: 13-Jan-2026 15:37:31
...
Model Annotations:
  ...
  Input sampling frequency: 4MHz
  SFDR: 80
  Mixer mode: Dual Input Mode

  p, li { white-space: pre-wrap; }
  Observation on Mixer output:
  Mixer output signal frequency1 :  (Input signal frequency1 + DDS signal frequency) = (200KHz + 100KHz) = 300KHz
  Mixer output signal frequency2 :  (Input signal frequency1 - DDS signal frequency) = (900KHz - 100KHz) = 800KHz
  Output sampling rate: 4MHz
  ...
  Each data sample of the second data input is complex multiplied with the conjugate (which is equivalent to a signal rotating in the opposite direction)  of the DDS sample to create a second modulated signal. These two modulated signals are added together and written to the output window.
...
% Quick Start: Mixer_LUT_Ex2
% Essential setup focusing on critical parameters

modelName = 'Mixer_LUT_Ex2';
new_system(modelName); open_system(modelName);
...
Model Annotations:
  ...
  SSR: 1
  Mixer mode: Single Input Mode
  Reloading Initial Phase via input port
...
% Quick Start: Mixer_LUT_Ex3
% Essential setup focusing on critical parameters

modelName = 'Mixer_LUT_Ex3';
new_system(modelName); open_system(modelName);
