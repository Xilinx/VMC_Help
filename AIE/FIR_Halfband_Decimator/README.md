...
Generated: 13-Jan-2026 15:37:19
...
Model Annotations:
  ...
  Output: Complex Signals Spectrum with 2 KHz & 10 KHz frequencies

  p, li { white-space: pre-wrap; }
  Model parameter settings:
  Input Frame Size = 1024
  Input Sampling Rate = 100 ksps
  Input: Complex Signals with 2 KHz & 10 KHz frequencies

  p, li { white-space: pre-wrap; }
  Note: Run above 2 lines of MATLAB commands in the command window to generate filter coefficients

  p, li { white-space: pre-wrap; }
  MATLAB Commands to Generate Filter Coefficients:
  HalfBandDecimator  = (firhalfband('minorder', 10e3/(100e3/2), 0.00001));
  Filter_coefficients = HalfBandDecimator([ 1:2:(length(HalfBandDecimator)-1)/2 (length(HalfBandDecimator)+1)/2]);
...
% Quick Start: HalfBandDecimator_Ex1
% Essential setup focusing on critical parameters

modelName = 'HalfBandDecimator_Ex1';
...
% CRITICAL: FIR Halfband coefficients use compressed format
% Formula: (N+1)/4+1 elements, where N is filter length
% Example: For 31-tap filter, provide 9 coefficients:
%   full_coeffs = firhalfband(30, 0.4);
%   Filter_coeffs = [full_coeffs(1:4:end-1), full_coeffs(16)];
...
Generated: 13-Jan-2026 15:37:20
...
Model Annotations:
  ...
  Input Sampling Rate = 100 ksps
  Input: Real Signals with 2 KHz & 10 KHz frequencies
  ...
  p, li { white-space: pre-wrap; }
  Observation on Filtered ouput:
  Output Frame Size = 256
  Output Sampling Rate = 50 ksps
  Output: Real Signal Spectrum with 2 KHz & 10 KHz frequencies
...
% Quick Start: HalfBandDecimator_Ex2
% Essential setup focusing on critical parameters

modelName = 'HalfBandDecimator_Ex2';
new_system(modelName); open_system(modelName);
