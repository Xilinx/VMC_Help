...
Generated: 13-Jan-2026 15:36:48
...
Model Annotations:
  ...
  F Input data type: cint16
  G Input data type: int16
  ...
  p, li { white-space: pre-wrap; }
  Note: The AIE Correlation output buffer sample size is currently set to (F_Length + G_Length).
  However, in 'Full' mode, the expected output sample size is (F_Length + G_Length - 1).
  Therefore, we need to discard the last sample when comparing with the reference.

  p, li { white-space: pre-wrap; }
  correlation(F,G) = convolution(F, conjugate(flip(G))​)
...
% Quick Start: Correlation_Ex1
% Essential setup focusing on critical parameters

modelName = 'Correlation_Ex1';
...
% Correlation
add_block('aieDSP/Correlation', [modelName '/Correlation']);
set_param([modelName '/Correlation'], ...
...
Model Annotations:
  p, li { white-space: pre-wrap; }
  Model Parameter Settings:
  F Input data type: float
  G Input data type: float
...
% Quick Start: Correlation_Ex2
% Essential setup focusing on critical parameters

modelName = 'Correlation_Ex2';
new_system(modelName); open_system(modelName);
...
Generated: 13-Jan-2026 15:36:49
...
Model Annotations:
  ...
  p, li { white-space: pre-wrap; }
  Note:  correlation(F,G) = convolution(F, conjugate(flip(G))​)
...
% Quick Start: Correlation_Ex3
% Essential setup focusing on critical parameters

modelName = 'Correlation_Ex3';
new_system(modelName); open_system(modelName);
...
% Quick Start: Correlation_Ex4
% Essential setup focusing on critical parameters

modelName = 'Correlation_Ex4';
new_system(modelName); open_system(modelName);
